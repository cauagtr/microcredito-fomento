...
export default function ProcessarPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setResult({ type: 'success', message: 'Planilha gerada com sucesso!', fileName, downloadUrl: url, dateTime })
      ...
      setResult({ type: 'error', message: 'Erro de conexão. Verifique sua internet e tente novamente.' })
    } finally {
      setProcessing(false)
    }
  }
    ...
    <div style={{ minHeight: '100vh', background: '#F8F9FB' }}>
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 24px 60px' }}>
        <div style={{ marginBottom: 32 }}>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: '#1A1F2E', letterSpacing: '-0.02em', margin: 0 }}>Processar Planilhas</h1>
          <p style={{ fontSize: 14, color: '#6B7A94', margin: '6px 0 0', lineHeight: 1.5 }}>Faça upload das planilhas exportadas do sistema e gere a consolidação diária automaticamente.</p>
        </div>
            ...
            <>
              {isFirstDay !== null && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: isFirstDay ? '#FFF8ED' : '#EBF4FF', border: `1px solid ${isFirstDay ? '#F5C97A' : '#BFDBFE'}`, borderRadius: 8, padding: '10px 16px', marginBottom: 28, fontSize: 13, color: isFirstDay ? '#92600A' : '#1D4ED8' }}>
                  <Info size={15} />
                  {isFirstDay ? 'Primeiro uso detectado — envie as 3 planilhas abaixo para iniciar o histórico.' : `Modo diário — a planilha anterior (${lastFileName || 'mais recente'}) será carregada automaticamente do histórico.`}
                </div>
              )}
              ...
              <button onClick={handleSubmit} disabled={!canSubmit || processing} style={{ width: '100%', background: !canSubmit || processing ? '#A0ABB8' : '#C8963E', color: '#FFF', border: 'none', borderRadius: 10, padding: '15px 24px', fontSize: 15, fontWeight: 600, cursor: !canSubmit || processing ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, letterSpacing: '-0.01em' }}>
                {processing ? (<><Loader2 size={18} style={{ animation: 'spin 0.8s linear infinite' }} />Processando...</>) : 'Gerar Planilha de Hoje'}
              </button>
              <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
              {result && (
        ...
        <div style={{ marginTop: 24, padding: '16px 20px', background: '#F0F4F8', borderRadius: 10, border: '1px solid #DDE3EE', fontSize: 12.5, color: '#6B7A94', lineHeight: 1.6 }}>
          <strong style={{ color: '#2C3649' }}>Como funciona:</strong>{' '}O sistema processa as planilhas exportadas, aplica os filtros e reordenamentos necessários, realiza o cruzamento de dados com o dia anterior e gera o arquivo consolidado com cores por colaborador.
        </div>
      </main>
    </div>
  )
